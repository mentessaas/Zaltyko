...
import { NON_INDEXABLE_ACADEMY_ROBOTS_HEADER } from "@/lib/seo/academy-robots";
...
function getPublicAcademyId(pathname: string): string | null {
  const match = pathname.match(/^\/academias\/([0-9a-f-]{36})\/?$/i);
  return match?.[1] ?? null;
}

async function shouldNoIndexAcademy(request: NextRequest): Promise<boolean> {
  const academyId = getPublicAcademyId(request.nextUrl.pathname);
  if (!academyId) return false;

  try {
    const response = await fetch(
      new URL(`/api/public/academies/${academyId}`, request.url),
      { headers: { accept: "application/json" }, cache: "no-store" }
    );

    // The public endpoint excludes isSuspended, churned and fraud_hold. A
    // confirmed 404 is therefore a non-indexable detail page. Fail closed on
    // upstream failures so a terminal page is never left indexable because a
    // status lookup was temporarily unavailable.
    return !response.ok;
  } catch {
    return true;
  }
...
export async function middleware(req: NextRequest) {
  ...
  if (await shouldNoIndexAcademy(req)) {
    response.headers.set("X-Robots-Tag", NON_INDEXABLE_ACADEMY_ROBOTS_HEADER);
  }
