...
- Authenticated academy checks require `E2E_ACADEMY_ID`; first run `pnpm test:e2e:verify-supabase` to confirm project keys and optional test-user login, then generate a saved browser session with `BASE_URL=http://127.0.0.1:3000 E2E_AUTH_EMAIL=... E2E_AUTH_PASSWORD=... pnpm test:e2e:auth`, then run audits with `E2E_STORAGE_STATE=.auth/user.json`.
