...
export async function calculateGrMetrics(params: GrMetricsParams): Promise<GrDashboardMetrics | undefined> {
  ...
  let upcomingCompetitions: UpcomingCompetition[] = [];
  try {
    ...
    upcomingCompetitions = upcomingCompetitionsResult.map((e) => ({
      id: e.id,
      title: e.title,
  ...
  try {
    ...
    assessmentsThisMonth = Number(assessmentsThisMonthResult[0]?.count ?? 0);
  } catch {
