...
/**
 * Tool callback type that accepts custom auth context
 * Note: The SDK expects RequestHandlerExtra but we use McpAuthContext
 */
type ToolCallback<T> = (args: T, context?: McpAuthContext) => Promise<{ content: { type: 'text'; text: string }[] }>;

// Helper to register tools with proper typing while bypassing SDK signature mismatch
const registerTool = <T extends Record<string, z.ZodTypeAny>>(
  server: McpServer,
  name: string,
  description: string,
  schema: T,
  handler: ToolCallback<z.infer<z.ZodObject<T>>>
) => {
...
export function registerAcademyTools(server: McpServer) {
   ...
   */
  registerTool(server, 'get_academy_info', 'Obtiene información completa de una academia por ID', {
...
${academy.trialEndsAt ? `- Trial termina: ${academy.trialEndsAt.toISOString()}` : ''}
   ...
   */
  registerTool(server, 'list_academies', 'Lista academias con filtros opcionales (país, región, ciudad, tipo)', {
   ...
   */
  registerTool(server, 'get_academy_athletes', 'Obtiene la lista de atletas de una academia', {
    academyId: z.string().uuid(),
   ...
   */
  registerTool(server, 'get_academy_classes', 'Obtiene las clases programadas de una academia', {
    academyId: z.string().uuid(),
   ...
   */
  registerTool(server, 'get_academy_financial_metrics', 'Obtiene métricas financieras de una academia (ingresos, pagos pendientes, etc.)', {
    academyId: z.string().uuid(),
...
📊 **Total facturado:** €${(revenue + pending).toFixed(2)}
   ...
   */
  registerTool(server, 'get_academy_events', 'Obtiene eventos de una academia', {
    academyId: z.string().uuid(),
    limit: z.number().int().min(1).max(100).default(20),
  }, async ({ academyId, limit }) => {
    try {
   ...
   */
  registerTool(server, 'search_academies', 'Busca academias por nombre (búsqueda parcial, case-insensitive)', {
