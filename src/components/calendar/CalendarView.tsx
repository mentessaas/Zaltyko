...
const statusColors: Record<string, string> = {
  scheduled: "bg-zaltyko-teal/12 text-zaltyko-teal",
  completed: "bg-zaltyko-indigo/10 text-zaltyko-indigo",
  cancelled: "bg-zaltyko-coral/12 text-zaltyko-coral",
  placeholder: "border border-dashed border-zaltyko-coral/35 bg-zaltyko-coral/10 text-zaltyko-coral",
};
    ...
    const content = (
      <>
        <div className="flex items-start justify-between gap-2">
          ...
          {session.isPlaceholder && (
            <AlertCircle className="h-3 w-3 shrink-0 text-zaltyko-coral" />
          )}
        </div>
        {session.startTime && (
        ...
        {session.isPlaceholder && (
          <div className="mt-1 rounded bg-zaltyko-coral/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-zaltyko-coral">
            No generada
          </div>
        )}
      </>
    ...
    <div className="space-y-6">
      ...
      {currentView === "agenda" ? (
                ...
                >
                  ...
                  <div className="mb-3 flex items-center justify-between">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={cn(
                            "text-lg font-bold",
                            isDayToday && "text-zaltyko-teal"
                          )}
                        >
                          {formatDateForCountry(day, academyCountry, "d")}
                        </span>
                        {isDayToday && (
                          <span className="rounded-full bg-zaltyko-teal px-2 py-0.5 text-[10px] font-semibold text-white">
                            Hoy
                          </span>
                        )}
                      </div>
                    ...
                    {daySessions.length > 0 && (
                      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-zaltyko-teal/10 text-xs font-semibold text-zaltyko-teal">
                        {daySessions.length}
                      </span>
                    )}
                  </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-zaltyko-mist bg-white shadow-soft">
          ...
          {/* Grid del mes */}
          <div className="grid grid-cols-7 gap-px bg-zaltyko-mist/70">
                    ...
                    <div className="mb-1 flex items-center justify-between">
                      <span
                        className={cn(
                          ...
                          isDayToday && "flex h-6 w-6 items-center justify-center rounded-full bg-zaltyko-teal text-white"
                        )}
                      >
                        {formatDateForCountry(day, academyCountry, "d")}
                      </span>
                      {daySessions.length > 0 && (
                        <span className="text-[10px] font-semibold text-zaltyko-teal">
                          {daySessions.length}
                        </span>
                      )}
                    </div>
