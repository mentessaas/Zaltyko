...
export function BillingPanel({ academyId, userId, sportConfigs = [] }: BillingPanelProps) {
  ...
  return (
    <div className="space-y-8">
      {error && (
        <p className="rounded-2xl border border-zaltyko-coral/35 bg-zaltyko-coral/10 p-4 text-sm text-zaltyko-coral">
          {error}
        </p>
      )}
                      ...
                      <td className="px-4 py-3 text-right whitespace-nowrap">
                        {invoice.hostedInvoiceUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 text-xs text-zaltyko-teal hover:underline"
                          >
                            <svg
