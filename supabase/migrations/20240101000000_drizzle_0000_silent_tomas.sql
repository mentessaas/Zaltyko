...
CREATE TABLE "academies" (
	...
	-- ZAL-336 [E2E] — columnas que existen en el schema Drizzle vivo pero
	-- que el repo no materializa en migraciones reproducibles (se añadieron
	-- out-of-band a prod). Las necesitamos aquí para que
	-- `createAcademy` (`/api/onboarding/owner`) pueda insertar.
	"country_code" text,
	"discipline" text,
	"discipline_variant" text,
	"federation_config_version" text,
	"specialization_status" text
);
--> statement-breakpoint
...
--> statement-breakpoint
-- ZAL-336: skipped auth.users (Supabase local provides it)
...
-- ZAL-336 [E2E] — Helper functions used by drizzle 0001+ RLS policies.
