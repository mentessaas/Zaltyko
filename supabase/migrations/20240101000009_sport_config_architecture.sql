-- ZAL-336 local E2E support: the real owner onboarding endpoint seeds the
-- sport catalog before it creates starter groups/classes. Keep this additive
-- migration after the baseline and before the app starts; it is not a
-- production migration or a substitute for the production ledger.

-- The owner setup route uses these billing-event columns when it creates the
-- free subscription. They are additive test-schema compatibility columns;
-- billing behavior is outside this E2E.
...
alter table groups add column if not exists technical_focus text;
alter table groups add column if not exists apparatus text[];
alter table groups add column if not exists session_blocks text[];
...
alter table classes add column if not exists technical_focus text;
alter table classes add column if not exists apparatus text[];
alter table classes add column if not exists is_extra boolean not null default false;
alter table classes add column if not exists group_id uuid references groups(id) on delete set null;
alter table classes add column if not exists auto_generate_sessions boolean not null default false;
alter table classes add column if not exists auto_generate_frequency text default 'monthly';
alter table classes add column if not exists last_auto_generated_at timestamptz;
alter table classes add column if not exists auto_generate_days_ahead integer default 30;
alter table classes add column if not exists allows_free_trial boolean not null default false;
alter table classes add column if not exists waiting_list_enabled boolean not null default false;
alter table classes add column if not exists cancellation_hours_before integer default 24;
alter table classes add column if not exists cancellation_policy text default 'standard';
