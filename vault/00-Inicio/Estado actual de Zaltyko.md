...
2. **Tablas TS faltantes en DB** (P0 parcialmente resuelto, verificado 2026-07-03 contra prod). **6 tablas críticas creadas y aplicadas a prod** (transacción, verificado): `class_exceptions` (corregida en `0006`: la FK apuntaba a `academies(tenant_id)` no única → nunca se aplicó; ahora `academies(id)` + RLS live), y `message_templates`/`message_groups`/`scheduled_notifications`/`notification_preferences`/`message_history` (`20260703000001` + `20260703000002`). Con esto **los 2 crons rotos quedan arreglados** (`generate-sessions`, `scheduled-notifications`) y el módulo de mensajería opera. `push_tokens` descartada (superseded por `push_subscriptions`). **Quedan ~15 tablas** de features laterales que requieren decisión de producto antes de materializar (rúbricas/tipos/videos de evaluación, `federative_licenses`, `leads`, `academy_roles`/`role_members`, `competition_results`, `scheduled_reports`, `leak_action_history`, sub-tablas `template_*`): no bloquean operación core ni onboarding. `drizzle-kit push --force` sigue prohibido; usar migración manual con el patrón transaccional + RLS live ya establecido.
...
1. P0: cerrar decisión legacy `/dashboard/*`. Migración de tablas faltantes: 6 críticas ya aplicadas (crons + mensajería OK); decidir qué features laterales (~15 tablas) se materializan según prioridad de producto.
