...
## 2026-08-05 - ZAL-336 E2E Playwright: signup con UTM completo → fila en `academies`

Cubre el último criterio de aceptación de ZAL-157 que la revisión ZAL-198 marcó como obligatorio: el spec debe atravesar el signup real (Supabase auth + onboarding) y verificar la fila persistida en `academies`. Cuatro escenarios, todos verdes en localhost contra Supabase local (Postgres 17 + GoTrue) + Next.js dev server.

**Estrategia de harness elegida (opción 1 del issue).** Supabase local completo (`supabase/config.toml` + `pnpm supabase start`) con schema mínima derivada de `drizzle/0000_silent_tomas.sql` + las migraciones `0007` (UTM) y `0008` (canal_registro) reubicadas en `supabase/migrations/`. El dev server se arranca en paralelo con `.env.local` apuntando a `127.0.0.1:54321` (auth) y `127.0.0.1:54322` (DB). El mock del auth server-side se descartó porque oculta el contrato real del signup; el stack local es reproducible y no toca Supabase remoto. `playwright.zal336.config.ts` aísla la config (testDir ignorando `.claude/**`, CSP bypass para que `supabase.auth.signUp` pueda conectar a loopback, webServer con `reuseExistingServer`).

**Escenarios cubiertos** (`tests/e2e-zaltyko-utm-signup.spec.ts`):
1. **Five UTM + landing path** → fila con los cinco `utm_*` normalizados (`google_ads_latam` → snake_case + minúsculas) + `utm_landing_path` + `utm_captured_at` + `canal_registro='paid'` (regla cpc/google_ads del trigger BEFORE INSERT de 0008).
2. **Direct (sin UTM)** → fila con todos los UTM null + `canal_registro='direct'` + `utm_captured_at` null.
3. **Claim con seed first-touch preserva UTMs** → seed en `supabase/seed.sql` pre-crea academia con `utm_source='instagram'`, `utm_medium='social'`, etc.; el signup + claim vía endpoint HTTP debe mantener esos valores intactos (regla `hasAnyUtm()` del guard de owner-claim.ts).
4. **Second touch sin overwrite** → primer touch con UTMs paid, segundo touch con UTMs social; `sessionStorage` retiene el primer set y la fila persistida lo refleja.

**Limitación documentada del flow real (no del spec).** El `RegisterForm` llama `POST /api/onboarding/profile` tras `supabase.auth.signUp`, lo que crea el perfil con un `tenant_id` NUEVO que no matchea el del seed. `resolveUserHome` entonces envía al usuario a `/dashboard/academies` en lugar de `/onboarding/owner`, por lo que la `OwnerClaimCard` queda estructuralmente fuera del alcance del signup con perfil nuevo. La página `/onboarding/owner/page.tsx` además redirige si `home.destination !== "owner_setup"`. El escenario 3 sortea esto llamando directo a `/api/onboarding/owner/claim` con los cookies del browser context (mismo helper que invoca `OwnerClaimCard.tsx`), preservando la verificación del contrato de persistencia. Pendiente derivada: pedir al Engineering Lead que evalúe saltear `/api/onboarding/profile` cuando el email matchea una academia seed-claimable, para que la UI de claim vuelva a ser alcanzable desde el signup. Esa decisión no entra en ZAL-336 (criterio cumplido con el escenario vía endpoint); se documenta acá para evitar que el siguiente agente la redescubra.

**Bugs del propio spec detectados y corregidos en este heartbeat.** `expect.poll(fn).toMatchObject(matcher)` no devuelve el valor sondeado (devuelve el matcher), así que `expect(row.utm_captured_at).toBeNull()` reventaba con `Cannot read properties of undefined`. Fix: separar la aserción `toMatchObject` del fetch — el poll garantiza la forma, una segunda llamada a `readAcademyRow(email)` devuelve el valor tipado para las inspecciones posteriores. Patrón aplicado a escenarios 1, 2 y 4. `gotoPublic` y `registerAndWaitForAuthCallback` añadieron retry transitorio para `ERR_ABORTED` durante el primer compile JIT de Next.js (mismo patrón que `tests/e2e-zaltyko-p1-flows.spec.ts → gotoAcademy`); sin esto, el primer test de una serie fría falla aunque el spec sea correcto.
