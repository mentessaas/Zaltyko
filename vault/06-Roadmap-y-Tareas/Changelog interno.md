...
Rama `feat/cobros-cuotas-stripe-connect` (no mergeada aún). Construye la capa de cobro real sobre el ledger `charges` existente, manteniendo a la academia como merchant of record y a Zaltyko fuera de la custodia de fondos y de lo fiscal. Arquitectura aprobada: **Stripe Connect Standard + direct charges + tarjeta Stripe-hosted + ledger `charges` como fuente de verdad**. Bizum/efectivo/transferencia siguen como pago manual.
