...
Snapshot verificado el 2026-09-12: 318 handlers; 221 tenant, 40 bearer, 19 públicos, 12 super-admin, 10 deprecated, 10 cron, 4 webhooks y 2 dev. Hay 212 handlers mutantes, 208 cubiertos por rate limit, 190 archivos con al menos una capability, 219 con Zod/validación equivalente, 283 con respuesta estándar, 284 con error estándar, 178 con evidencia de scope de academia, 4 con service role y **0** recursos dinámicos pendientes de revisión manual. El único `tenantId` aceptado desde body está limitado al flujo legacy de super-admin en `/api/admin/users`; `/api/athletes` ya no acepta override de tenant.
...
| coach baseline | `athletes:read/update`, `classes:read/schedule`, `events:read`, `communications:read/send` | clase/sesión/atleta asignado cuando aplica; sin billing/settings/reportes ejecutivos |
...
| reports | `read`, `create`, `export` | ninguno en baseline; acceso administrativo explícito | 27 |
