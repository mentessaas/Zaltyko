...
- `SEC-006` quedó mitigado en código con `src/lib/uploads/file-security.ts`: allowlist centralizada, magic bytes, límites por tipo, rutas aleatorias y rechazo de la firma EICAR antes de persistir. Bucket privado y antimalware completo siguen requiriendo configuración externa.
...
- Supabase Storage: `uploads` y `ticket-attachments` son privados; `avatars` y `events` son públicos de forma intencional, con límites y allowlists por bucket provisionados por `20260911100000_provision_storage_buckets.sql`. No existe scanner antimalware externo conectado; el control de producción sigue abierto hasta incorporar uno.
...
| SEC-006 | `src/lib/uploads/file-security.ts`, `/api/upload`, `/api/assessments/videos` | **Mitigado en código:** allowlist centralizada, límites, firma binaria (magic bytes), MIME, extensión saneada, rutas únicas con `crypto.randomBytes` y rechazo EICAR. Sigue pendiente el escaneo antimalware externo para malware desconocido. | Media | Archivos polimórficos o malware avanzado requieren un control fuera del proceso web. | Configurar escaneo asíncrono en Supabase/worker y conservar el estado de cuarentena. | Sol |
...
- Supabase Storage mantiene `uploads` y `ticket-attachments` privados, con límites y allowlists; `avatars`/`events` son públicos para media explícitamente publicada. La descarga anónima de un objeto privado temporal devolvió 400 y el objeto fue eliminado. Los adjuntos nuevos usan referencias estables y URLs firmadas al leer un ticket autorizado; el escaneo antimalware externo sigue pendiente.
...
- El webhook Mailgun heredado exige timestamp dentro de cinco minutos, HMAC válido, comparación constante, escapa el contenido reenviado y reclama cada entrega en Vercel KV durante esa ventana (`src/lib/webhook-replay.ts`). Una repetición devuelve 409; si KV no está disponible en producción, el endpoint falla cerrado con 503.
...
- Los enlaces públicos de baja/preferencias generan nonces con `crypto.randomBytes(12)`; no dependen de `Math.random()` ni del reloj del proceso.
