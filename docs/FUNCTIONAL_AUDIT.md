...
| Comunicacion/recordatorios | Modulos messages, announcements, notifications, WhatsApp, templates y alerts existen. | Brevo y WhatsApp dependen de configuración externa; no validar envío real en demo si no están configurados. | Media | `src/app/app/[academyId]/messages`, `src/app/api/communication/*`, `src/app/api/whatsapp/*` | Preparar demo con comunicaciones internas/read-only o mocks claramente marcados. |
